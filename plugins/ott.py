...
async def get_or_fetch(date_obj: date) -> dict:
    ...
    if stored["movies"] or stored["series"]:
        # Past dates (>7 days ago): cache forever
        if days_old > 7:
            return stored
        # Future dates: re-fetch every 6 hours
        # Recent dates (within 7 days): re-fetch every 6 hours
        cache_age = await get_cache_age_hours(date_str)
        if cache_age < 168:
            return stored
        # Cache is stale — clear and re-fetch
        from database.ott_db import clear_releases_by_date
        await clear_releases_by_date(date_str)
