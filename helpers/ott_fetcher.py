...
async def _tmdb_get_episode_info(session, tmdb_id: int, target_date: date) -> Optional[str]:
    """
    Given a TMDB show ID, find which season/episode aired on target_date.
    Returns a string like 'S02E05', 'S03' (whole season premiere), or None.
    Strategy:
      1. Fetch show details to get list of seasons.
      2. Check the most recent season first (sorted by season_number desc).
      3. For each season, check if its air_date matches or if any episode does.
    """
    date_str = target_date.strftime("%Y-%m-%d")

    # Step 1: get show details with season list
    show_data = await _get(session, f"{TMDB_BASE}/tv/{tmdb_id}", {"api_key": TMDB_API_KEY})
    if not show_data:
        return None

    seasons = show_data.get("seasons", [])
    # Filter out specials (season 0) and sort newest first
    seasons = [s for s in seasons if s.get("season_number", 0) > 0]
    seasons.sort(key=lambda s: s.get("season_number", 0), reverse=True)

    for season in seasons:
        sn = season.get("season_number")
        season_air = season.get("air_date", "")

        # Skip seasons that haven't aired yet or aired way before target date
        if not season_air:
            continue
        try:
            season_date = datetime.strptime(season_air, "%Y-%m-%d").date()
        except ValueError:
            continue

        # Only look at seasons that could contain an episode on target_date
        if season_date > target_date:
            continue

        # Step 2: fetch episode list for this season
        season_data = await _get(
            session,
            f"{TMDB_BASE}/tv/{tmdb_id}/season/{sn}",
            {"api_key": TMDB_API_KEY},
        )
        if not season_data:
            continue
        ...
        for ep in episodes:
            ep_air = ep.get("air_date", "")
            if ep_air == date_str:
                ep_num = ep.get("episode_number")
                return f"S{sn:02d}E{ep_num:02d}"

        # If the season premiere itself is on target_date
        if season_air == date_str:
            return f"S{sn:02d}"
...
async def tmdb_tv_season_releases(session, target_date: date):
    """
    Get TV shows airing on a specific date via TMDB, with season/episode info.
    Two-step: discover which shows air → then look up S##E## for each.
    """
    date_str = target_date.strftime("%Y-%m-%d")
    ...
    # Fetch episode info for all shows concurrently (capped at 10 to avoid rate limits)
    async def _enrich(item):
        ...
        extra = ""
        if tmdb_id:
            extra = await _tmdb_get_episode_info(session, tmdb_id, target_date) or ""
        return {
            "source": "tmdb_tv",
            "title": title,
            "language": lang,
    ...
    enriched = await asyncio.gather(*[_enrich(item) for item in shows[:10]], return_exceptions=True)
...
def cross_validate(all_results: list) -> list:
    ...
    for key, entries in grouped.items():
        ...
        best = dict(entries[0])
        for e in entries[1:]:
            if not best.get("extra") and e.get("extra"):
                best["extra"] = e["extra"]
            cur_lang = best.get("language", "")
            new_lang = e.get("language", "")
            if len(new_lang) > 2 and len(cur_lang) <= 2:
                best["language"] = new_lang
