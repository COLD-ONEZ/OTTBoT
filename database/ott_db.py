...
async def get_cache_age_hours(date_str: str) -> float:
    """Returns how many hours ago this date was last fetched. 9999 if never."""
    from datetime import datetime
    doc = await releases_col.find_one({"date": date_str}, sort=[("fetched_at", -1)])
    if not doc or not doc.get("fetched_at"):
        return 9999.0
    diff = datetime.utcnow() - doc["fetched_at"]
